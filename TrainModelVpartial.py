...
ds = pd.read_excel("partial_utilities.xlsx")
...
x = ds.iloc[:,3].values
prey_belief = list(x)#:10]

output_scale=14#13.8594428241402
lookup_table={}
...
for i in range(len(key_state)):
    ...
    agent=int(l[0])
    predator=int(l[1])
    prey_prob=prey_belief[i].replace(" ","").replace("[","").replace("]","").split(",")
    inp=[len(shortest_path(agent,predator))-1]+prey_prob+[1]
    if utility[i]<50:
...
data_export=[list(lookup_table.keys()), list(lookup_table.values())]
...
# np.random.seed(1)
weight1 = 1 * np.random.randn(52, 5) #(input size*hiddenlayer1 size)
